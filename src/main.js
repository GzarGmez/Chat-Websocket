...
socket.on('login', ( ) => {
    alert('Bienvenido al Chat de la Universidad Politécnica de Chiapas')
...
socket.on('send-private-message', ({from, message, image}) => {
    messagesList.insertAdjacentHTML('beforeend',`<li>[personal] ${from}: ${message}</li>`);
    if(image!== undefined){
        const imagen = document.createElement("img")
        imagen.src = image
        messagesList.appendChild(imagen);
...
inputMessageButton.addEventListener('click', ()=>{
    if ( inputMessage.value.startsWith('/personal') ) {
...
photoInput.addEventListener('change', (e)=>{
    ...
    DataURL ? alert('Foto Adjuntada') : alert('Añade la imagen nuevamente para enviar');
})